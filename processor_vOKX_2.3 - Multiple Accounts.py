...
rob_api_key = '20cf3fd4-24b1-4a8d-86f2-7240a105a228'
rob_secret = 'DFBF64A3D572AF49D4728A1F676A99A1'
rob_pass = 'SpittingBars12!'

dad_api_key = '36dac7f3-8659-4369-acac-e8518a4d3d6a'
dad_secret = '672D4E855B61539ACBE5D2D5677D18D7'
dad_pass = 'At229z3Na$eg'
		...
		if chatlog != content:
			...
			if filterMessages(chatlog):
				...
				if stoploss < buyprice:
					#print(data_dict)
					connectAPI(data_dict, rob_api_key, rob_secret, rob_pass, '0')
					connectAPI(data_dict, dad_api_key, dad_secret, dad_pass, '0')
...
def sendNotif(name, price, size, lever, stoploss, takeprofit):
	nameLine = 'ASSET NAME: ' + name
	priceLine = 'ASSET PRICE: ' + str(price)
	sizeLine = 'ORDER SIZE: ' + str(size)
	leverLine = 'LEVERAGE: ' + str(lever)
...
def connectAPI(params, api_key, secret_key, passphrase, flag):
	flag = flag # 0 for live trading
	api_key = api_key
	...
	if not position_open and not abandon:
		...
		orderTable(stripped_coin, buyprice, (int(num_of_contracts) * int(contract_value_usd)), leverage, stoploss, take_profit)
		sendNotif(stripped_coin, buyprice, (int(num_of_contracts) * int(contract_value_usd)), leverage, stoploss, take_profit)
